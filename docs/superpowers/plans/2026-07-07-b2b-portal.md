# Confire B2B Client Portal Implementation Plan
...
**Interfaces:**
- Produces: `portalClient` table; `configModel.portal: boolean`, `configModel.portalDescription: text`; `ProjectStatus = "draft" | "calculated" | "quoted" | "requested" | "rejected"`; `ProjectSource`; `ProjectEvent`; `configProject.source/rejectionNote/events` columns. All later server tasks import these from `@confire/db`.
...
Rewrite the middle of `apps/server/src/orpc/base.ts` (keep `InitialContext`, `base`, `requireSession`, and the agent layer as-is). Add `portalClient` to the `@confire/db` import and export `baseDomain`:

```ts
import { db, tenantIntegration, member, organization, portalClient } from "@confire/db";
// ...
...
import { randomBytes } from "node:crypto";
import { db, configModel, member, organization, portalClient, user } from "@confire/db";
...
**Interfaces:**
- Consumes: `EntriesZ` from `@confire/config-engine`; `configProject`, `configRun` from `@confire/db`.
- Produces:
...
Add imports: `sql`, `inArray` from `drizzle-orm`; `configProject`, `configRun` from `@confire/db`; `EntriesZ` from `@confire/config-engine`; `ProjectEvent` type from `@confire/db`.
...
**Interfaces:**
- Consumes: `executeRun`, `loadModel`, `freshLookups`, `needsAgent` (configs.ts); `assertAgentReady` (entities.ts); `agentFetcher` (models.ts); `RunCandidate` (`@confire/db`).
- Produces:
...
Add `import type { ModelDef, ResolvedLookups } from "@confire/config-engine";`. Verify: `bun test apps/server` still green (no behavior change).

- [ ] **Step 5: Add the sanitizers + `get`/`run`/`lookups`/`extract` to `portal.ts`**

New imports: `loadModel, cachedLookups, executeRun, freshLookups, needsAgent` from `./configs.ts`; `assertAgentReady` from `./entities.ts`; `agentFetcher` from `./models.ts`; `ExtractFileZ, extractSuggestions` from `./extraction.ts`; `type RunCandidate` from `@confire/db`; `type Entries, type ModelDef` from `@confire/config-engine`.
...
Add `sql` to the drizzle imports, `user` and `type ProjectEvent` to the `@confire/db` import.
...
```ts
import type { ProjectStatus } from "@confire/db";
...
Note: `apps/web` doesn’t currently depend on `@confire/db` — and per project memory, bun isolated installs need direct deps. **Don’t add the dep**; inline the status union instead:
...
**Interfaces:**
- Consumes: `orpc.portal.projects.get/update`, `orpc.portal.run/lookups/extract/submit/withdraw/reopen/quotedResult`; `StepConfigure`, `StepBatches`, `StepCandidates` (Task 9 shape), `propagate` from `@confire/config-engine`; `portalStatusUi`.
